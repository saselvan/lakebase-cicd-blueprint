...
# It regenerates the bundle resources into an ISOLATED temp bundle (never the committed tree), so a
# missing/broken codegen step surfaces as absent resources here, not as a stale committed file that
# would mask the bug. Validation runs in --strict mode (warnings are errors), against a localhost
# stub that answers the single SCIM `Me` / get-status call the CLI makes — no real workspace needed.
#
# Falsifiability (the reviewer's three mutations):
...
#   3. Remove the codegen step below              -> temp/resources is empty  -> the count check exits non-zero.
#
...
# --- 1) Codegen (single source of truth -> bundle resources). Mutation 3 removes this line. ---
