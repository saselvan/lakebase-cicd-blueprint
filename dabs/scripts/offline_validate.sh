...
# --- 4) Count check: the bundle must carry one synced table per config row and NO role resource.
# The migration owns the app role (guarded CREATE ROLE), so no postgres_roles resource is emitted;
# a re-introduced role resource fails this check by design.
...
if [ "$n_synced" != "$expected" ] || [ "$n_roles" != "0" ]; then
  echo "ERROR: expected $expected synced tables and 0 role resources; got synced=$n_synced roles=$n_roles" >&2
  echo "       (did the codegen step run? absent/stale resources — or a re-introduced role resource — fail here.)" >&2
  exit 1
fi

echo "OFFLINE VALIDATE OK: both targets strict-validated; $expected table(s) fully generated; no role resource."
