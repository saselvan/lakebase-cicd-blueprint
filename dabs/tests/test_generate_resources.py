...
def test_one_synced_table_per_row_and_no_roles(tmp_path):
    """N rows -> exactly N postgres_synced_tables and ZERO postgres_roles (the migration owns the
    role).

    Guards mutation 1: replacing the loop with "emit first table only" drops the count to 1 and
    turns this red. Also guards a re-introduced role resource (roles must stay empty).
    """
    out = _generate(tmp_path)
    n = len(_fixture_rows())
    assert len(out["synced"]) == n, f"expected {n} synced tables, got {list(out['synced'])}"
    assert out["roles"] == {}, f"expected zero role resources, got {list(out['roles'])}"
...
def test_invalid_scheduling_policy_is_rejected(tmp_path):
    """A `scheduling_policy` outside the bundle-schema enum (CONTINUOUS|TRIGGERED|SNAPSHOT) makes
    generation raise — codegen never emits a policy the synced-table CREATE API would refuse.

    MUTATION GATE: drop the enum validation and this goes RED (an invalid policy sails through).
    """
    from dabs.generate_resources import load_tables, write_resources

    rows = json.loads(FIXTURE.read_text())
    rows[0]["scheduling_policy"] = "HOURLY"  # not in the enum
    cfg = tmp_path / "bad_policy.json"
    cfg.write_text(json.dumps(rows))
    with pytest.raises(ValueError) as exc:
        write_resources(load_tables(cfg), tmp_path / "out")
    assert "HOURLY" in str(exc.value), f"error must name the offending policy: {exc.value}"
...
def test_no_postgres_roles_resource_is_emitted(tmp_path):
    """The generator emits NO `postgres_roles` resource for any table — the migration OWNS the role.
    Two objects used to create the app role: this generated DAB resource AND the migration's guarded
    `CREATE ROLE`. Keeping only the migration means `bundle destroy` can never
    drop a role out from under existing grants, and both migration paths produce identical objects.

    MUTATION GATE: re-introduce `build_role_resource` / a `postgres_roles` emission and this goes RED
    (a role resource re-appears in the emitted YAML for every row).
    """
    out = _generate(tmp_path)
    assert out["roles"] == {}, (
        f"generator still emits postgres_roles resources: {list(out['roles'])} — "
        "the migration owns the role; no DAB role resource should be emitted"
    )
    # And, at the raw-file level, no emitted YAML declares a postgres_roles resource block at all.
    for f in out["files"]:
        doc = yaml.safe_load(Path(f).read_text())
        res = (doc.get("resources") or {})
        assert "postgres_roles" not in res, (
            f"{Path(f).name} still declares a postgres_roles resource: {res.get('postgres_roles')}"
        )
...
def test_drift_check_detects_stale_committed_file(tmp_path):
    ...
    assert "members_src" in text, "fixture regression: expected source table name in committed members.yml"
    victim.write_text(text.replace("members_src", "members_TAMPERED_src"))
