...
| After changing a table's **sync mode**, the app lost SELECT and its indexes | A sync-mode change forces a **full replace** (destroy+recreate) → drops custom indexes and grants to pkey-only | Because the grant/index/view changesets are `runAlways:true`, they reapply and restore access — *but* the full replace must drop the base table first, and a dependent **consumer view** (this reference always ships one) blocks that in-place drop. So this is a clean rebuild only when nothing depends on the table; otherwise **drop the consumer view first** (or do a blue/green swap) — see the "Changing a table's sync mode" row below. |
...
| Changing a table's sync mode (SNAPSHOT/TRIGGERED/CONTINUOUS) | Sync mode is a create-time property, so the change forces a full replace of the synced table — and the base table's dependent **consumer view** can block the drop (Postgres won't drop a table that still has dependents) | Prefer a **blue/green swap** — stand up a new table with the target mode, cut over, then drop the old one (see DESIGN-NOTES → "Changing sync mode: prefer a blue/green swap"). If you must replace in place, **drop the consumer view first**, then redeploy so `runAlways` recreates role/grants/indexes/view. |
